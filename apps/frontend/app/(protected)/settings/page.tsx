...
const THEMES = [
  ...
  },
  {
    ...
    preview: 'bg-gradient-to-br from-amber-400 via-orange-500 to-purple-600',
    text: 'Sunset',
  },
  {
    name: 'Nord',
  ...
  },
  {
    ...
    preview: 'bg-gradient-to-br from-emerald-400 via-green-500 to-yellow-600',
    text: 'Emerald',
  },
  {
    name: 'Dracula',
...
export default function SettingsPage() {
  ...
  useEffect(() => {
    const storedTheme = sessionStorage.getItem('customTheme') || 'theme-default'
    ...
    <div className="max-w-2xl mx-auto py-10">
      ...
      <Card>
        <CardHeader>
          ...
          <CardDescription>Manage your profile and personalize your experience. <span className="text-muted-foreground">(Profile is saved, theme is session-only)</span></CardDescription>
        </CardHeader>
        <CardContent>
          <Tabs value={tab} onValueChange={setTab} className="w-full">
            ...
            <TabsContent value="appearance">
              <div className="grid gap-8">
                <div className="grid gap-2">
                  <Label><Palette className="w-4 h-4" /> Theme</Label>
                ...
                <div className="grid gap-2">
                  <Label><Palette className="w-4 h-4" /> Color Theme</Label>
                    ...
                    {THEMES.map(opt => (
                      <button
                        ...
                        className={cn(
                          ...
                          'focus:outline-none',
                          customTheme === opt.className && 'ring-2 ring-primary'
                        )}
                        aria-label={opt.name}
                        onClick={() => setCustomTheme(opt.className)}
                      >
                        <div className={cn('w-12 h-8 rounded-lg shadow-sm', opt.preview, opt.className)} />
                        <span className="text-xs mt-1 text-muted-foreground">{opt.text}</span>
                        {customTheme === opt.className && (
                          <Check className="absolute top-1 right-1 w-4 h-4 text-primary bg-background rounded-full" />
                        )}
                      </button>
                    ))}
                  </div>
                  <div className="text-xs text-muted-foreground mt-1">Theme is stored in session and only affects this device.</div>
                </div>
              </div>
            </TabsContent>
          </Tabs>
        ...
        <CardFooter className="flex justify-end gap-2">
          {tab === 'profile' && (
            <Button onClick={handleSaveProfile} disabled={loading || updateStatus === 'pending'}>
              {loading || updateStatus === 'pending' ? <Loader2 className="animate-spin w-4 h-4 mr-2" /> : null}
              Save Profile
            </Button>
          )}
        </CardFooter>
