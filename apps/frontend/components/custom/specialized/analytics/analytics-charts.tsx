...
export function AnalyticsCharts({ className }: { className?: string }) {
    ...
    const chartData = React.useMemo(() => {
        ...
        invoices.forEach((invoice: Invoice) => {
            ...
            if (monthIndex !== -1) {
                ...
                if (invoice.state) {
                    const stateKey =
                        invoice.state.toLowerCase() as keyof (typeof months)[number]
    ...
    const statusData = React.useMemo(() => {
        ...
        invoices.forEach((invoice) => {
            if (invoice.state) {
                statusMap.set(
                    invoice.state,
                    (statusMap.get(invoice.state) || 0) + 1
                )
            }
        })
    ...
    const clientData = React.useMemo(() => {
        const clientMap = new Map<
            string,
            { name: string; total: number; paid: number; pending: number }
        >()

        invoices.forEach((invoice: Invoice) => {
            const client = clients.find(
                (c: Client) => c.id === invoice.client_id
            )
            ...
            if (client) {
                ...
                if (invoice.state === 'Paid')
                    clientEntry.paid += invoice.total_amount || 0
                if (invoice.state === 'Sent')
                    clientEntry.pending += invoice.total_amount || 0
            }
        })
                            ...
                            <PieChart accessibilityLayer>
                                ...
                                >
                                    {statusData.map((entry) => (
                                        <Cell
                                            key={entry.status}
                                            fill={`var(--color-${entry.status.toLowerCase()})`}
                                        />
                                    ))}
                                </Pie>
                                <ChartLegend
